...
// CẤU HÌNH FIREBASE CỦA TRUNG TÂM
...
// 🔒 MÃ PIN BÍ MẬT DÀNH RIÊNG CHO QUẢN LÝ TRUNG TÂM
  ...
  useEffect(() => {
    if (msgType === "done") {
      setMessage(
        `Dạ chào ${
          customer.name
        }, Trung tâm Veston Sĩ Hiền xin thông báo đơn hàng [Mã: ${
          customer.orderName
        }] của quý khách đã hoàn thiện.\n\nTổng tiền còn nợ: ${formatCurrency(
          customer.conLai
        )} VNĐ.\n\nMời quý khách ghé Trung tâm thử và nhận đồ nhé. Cảm ơn quý khách!`
      );
    } else {
      setMessage(
        `Dạ chào ${customer.name}, Trung tâm Veston Sĩ Hiền thành thật xin lỗi quý khách vì sự cố ngoài ý muốn nên đơn hàng [Mã: ${customer.orderName}] sẽ bị trễ hẹn so với dự kiến.\n\nTrung tâm đang gấp rút hoàn thiện và sẽ báo lại quý khách sớm nhất. Rất mong quý khách thông cảm!`
      );
    }
  }, [msgType, customer]);
